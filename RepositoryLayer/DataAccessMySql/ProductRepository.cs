...
namespace RepositoryLayer
{
    public class ProductRepository : IProductRepository
    {
        ...
        public Product GetProduct(int productId)
        {
            ...
            string sql = string.Format(@"SELECT ProductId, ProductName, EanCode, ImageUrl, Price, Length, Width, Height, Description FROM Products Where ProductId={0}", productId);
            using (MySqlDataReader reader = MysqlRepository.ExecuteReader(MysqlRepository.ConnectionString_ReadOnly, CommandType.Text, sql))
            {
                if (reader.Read())
                {
